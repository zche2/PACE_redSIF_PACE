...
function Retrieval_for_Pixel_xSecFit(
	...
	# println("[Retrieval] Starting pixel retrieval...")
	...
	# println("  Setup time: $(round(t_setup*1000, digits=2)) ms")

	# set-up
	t_priori = @elapsed begin
		MakePriori!(MyPixel, βₐ, γ=γₐ);
		MyPixel.x  = copy(MyPixel.xₐ);
	end
	# println("  Prior setup time: $(round(t_priori*1000, digits=2)) ms")
	
	t_fwd = @elapsed begin
		MyPixel.y  = MyModel(MyPixel.x, MyPixel);
	end
	# println("  Initial forward model time: $(round(t_fwd*1000, digits=2)) ms")
	...
	t_iter = @elapsed begin
		try
			...
			println("[Retrieval] Error during iteration: ", e)
			return missing
		end
	end
	# println("  Iteration time: $(round(t_iter*1000, digits=2)) ms ($(MyPixel.iter_label) iterations)")
	# println("  Total retrieval time: $(round((t_setup + t_priori + t_fwd + t_iter)*1000, digits=2)) ms")

	# Step3: return
	# return if converge
	if abs(MyPixel.ΔRMSE) < thr_Converge
		# println("[Retrieval] Successfully retrieved (ΔRMSE = $(MyPixel.ΔRMSE))")
		return MyPixel
	else
		# println("[Retrieval] Failed to converge (ΔRMSE = $(MyPixel.ΔRMSE))")
		return missing
	end
...
function Retrieval_for_Pixel_PCFit(
	...
	# println("[Retrieval PC] Starting pixel retrieval...")
	t_setup = @elapsed begin
		MyPixel       = Pixel_PCFit();
	...
	t_iter = @elapsed begin
		try
			...
			println("[Retrieval PC] Error during iteration: ", e)
			return missing
		end
	end
	# println("  Iteration time: $(round(t_iter*1000, digits=2)) ms ($(MyPixel.iter_label) iterations)")
	...
	# return if converge
	if abs(MyPixel.ΔRMSE) < thr_Converge
		# println("[Retrieval PC] Successfully retrieved (ΔRMSE = $(MyPixel.ΔRMSE))")
		return MyPixel
	else
		# println("[Retrieval PC] Failed to converge (ΔRMSE = $(MyPixel.ΔRMSE))")
		return missing
	end
