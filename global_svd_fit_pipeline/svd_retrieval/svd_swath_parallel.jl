...
function _create_output_dataset(
    ...
    if save_posterior
        # Posterior variance only (diag of Σ); full n_state×n_state matrix is omitted to save space.
        v_spost = defVar(ds, "S_posterior_diag", Float32, ("pixels", "scans", "state"); comp...)
        v_spost.attrib["long_name"] = "posterior variance (diagonal of S_posterior)"
        v_spost.attrib["formula"] = "diag(inv(J' * Se_inv * J + Sa_inv)) at final x_hat"
    end
...
function _build_svd_swath_shared(interim_nc::AbstractString, l1b_path::AbstractString, retrieval_cfg::AbstractDict)
    ...
    summer_nc = get(data_cfg, "summer_nc", get(svd_cfg, "summer_nc", nothing))
    winter_nc = get(data_cfg, "winter_nc", get(svd_cfg, "winter_nc", nothing))
    (summer_nc !== nothing && winter_nc !== nothing) ||
        error("Set [data].summer_nc and [data].winter_nc (or legacy [fit.svd] summer_nc / winter_nc)")
    summer_nc = String(summer_nc)
    winter_nc = String(winter_nc)
    summer_abs = isabspath(summer_nc) ? summer_nc : joinpath(base_dir, summer_nc)
    winter_abs = isabspath(winter_nc) ? winter_nc : joinpath(base_dir, winter_nc)
    n_pc = Int(get(svd_cfg, "n_pc", 5))
    n_leg = Int(get(svd_cfg, "n_legendre", 3))
    log_trans = Bool(get(svd_cfg, "svd_log_transform", false))
    sif_rel = get(data_cfg, "sif_file", nothing)
    sif_rel === nothing && error("Set [data].sif_file (JLD2 SIF basis); SVD path does not use LUT/kernel preparation.")
...
function _process_one_pixel_svd!(
    ...
    if spost_diag_scan !== nothing
        spost_diag_scan[i_pix_out, :] .= Float32.(diag(stats.S_posterior))
    end
...
function _flush_svd_tile!(
    ...
    spost_diag_scan::Union{Nothing, Matrix{Float32}},
    ...
    for k in 1:K
        ...
        H_obs = Jk' * Se * Jk
        Spost, post_ok = _posterior_covariance(H_obs, S_a_inv)
        if spost_diag_scan !== nothing
            spost_diag_scan[io, :] .= Float32.(diag(Spost))
        end
        if !post_ok
            status_scan[io] = Int16(4)
            conv_scan[io] = UInt8(0)
            dof_scan[io] = Float32(NaN)
            rchi2_scan[io] = Float32(NaN)
        else
...
function run_svd_orbit_full_nc_parallel(
    ...
    save_posterior = Bool(get(batch_cfg, "save_posterior", true))
    ...
    println("  prefetch_input: ", prefetch_input, "  deferred_write: ", deferred_write,
        "  compression_level: ", compression_level, "  save_posterior: ", save_posterior)
    ...
    buf_spost_diag = (deferred_write && save_posterior) ?
        fill(Float32(NaN), n_pix_out, n_scan_out, n_state) : nothing
    ...
    for (j_scan_out, j_scan_src) in enumerate(scan_range)
        ...
        spost_diag_scan = save_posterior ?
            fill(Float32(NaN), length(pixel_range), sh.layout.n_state) : nothing
        ...
        t_write_total += @elapsed if deferred_write
            ...
            if save_posterior
                buf_spost_diag[:, j_scan_out, :] = spost_diag_scan
            end
            ...
            if save_posterior
                ds_out["S_posterior_diag"][:, j_scan_out, :] = spost_diag_scan
            end
    ...
    if deferred_write
        t_bulk_write = @elapsed begin
            ...
            if save_posterior
                ds_out["S_posterior_diag"][:, :, :] = buf_spost_diag
            end
