...
"""
    prepare_mwe_inputs(cfg::AbstractDict)

Loads LUTs, SIF basis, and kernel from an already-parsed config dictionary (same logic as the path-based overload).
...
"""
function prepare_mwe_inputs(cfg::AbstractDict)
...
"""
    prepare_mwe_inputs(config_path::AbstractString)

Reads TOML from disk, then [`prepare_mwe_inputs(cfg)`](@ref).
"""
function prepare_mwe_inputs(config_path::AbstractString)
    return prepare_mwe_inputs(read_mwe_config(config_path))
end
