...
begin
	summer = Dataset("/home/zhe2/data/MyProjects/PACE_redSIF_PACE/convolved_transmittance/transmittance_summer_20240705.nc");
	winter = Dataset("/home/zhe2/data/MyProjects/PACE_redSIF_PACE/convolved_transmittance/transmittance_winter_20231230.nc");
	...
	trans = cat(summer["transmission"][:,:], winter["transmission"][:,:], dims=1);
	println("\nConcatenated!")
...
begin
	...
	p = plot(bands[ind₂], U[:,1],
			 label = "PC1 ($(round.(S_norm[1], digits=3))%)",
			 subplot = 1,
			 legend=:outerright,
			 layout = MyLayout,
			 size=(800, 800),
	...
	for i in 2:nPC
		plot!(p, bands[ind₂], U[:, i],
				 label = "PC$i ($(round.(S_norm[i], digits=3))%)",
				 subplot = i,
				 legend=:outerright,
				 layout = MyLayout,
		)
	end
	plot!(plot_title=edit_title, plot_titlefontsize=12)
